...
# >>>>>>>>>>> Global Settings <<<<<<<<<<<
# India job hunt tips: keep `stealth_mode = True`, set `search_location = "India"` in search.py,
# and tune `enforce_india_salary_rules` / stipend & LPA floors there. Use `run_non_stop = True` only if you want repeated cycles (long runs).
