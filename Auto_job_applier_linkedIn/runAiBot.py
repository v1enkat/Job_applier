...
def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool]:
    ...
    * skip: A boolean flag to skip this job
    ...
    # Skip only truly unavailable *promoted* listings (URL flag). Do NOT use substring "disabled" on
    # class names — tokens like "not-disabled" contain that substring and caused mass false skips.
    try:
        link_href = (job_details_button.get_attribute("href") or "").lower()
        if "budget_exhausted_job" in link_href or "ebp=budget_exhausted" in link_href:
            print_lg(f'Skipping "{title} | {job_id}" — promoted listing unavailable (budget exhausted in URL).')
            skip = True
            return (job_id, title, "", "", "", skip)
    except Exception:
        pass
    ...
    try:
        ...
        try:
            ...
            pass
        # Do not re-click (was causing repeated errors and driver/session loss on disabled overlays).
        print_lg("Skipping this job card and continuing.")
        skip = True
    buffer(click_gap)
...
def _parse_inr_amount(raw: str) -> int:
    '''Parse "12000", "12,000", "15k", "8.5k" into integer INR (monthly-ish scale).'''
...
def india_compensation_gate(job_title: str, job_description: str) -> tuple[bool, str | None, str | None]:
    '''
    Optional India-focused skip when JD clearly states stipend/CTC below configured floors.
    Returns (skip, skipReason, skipMessage).
    '''
    if not enforce_india_salary_rules:
        return False, None, None
    blob = f"{job_title}\n{job_description}"
    low = blob.lower()
    intern_like = bool(re.search(r'\b(internship|research\s+intern|student\s+intern|summer\s+intern|winter\s+intern)\b', low))
    intern_like = intern_like or (bool(re.search(r'\bintern\b', job_title.lower())) and 'internal' not in job_title.lower())

    if intern_like:
        for m in re.finditer(
            r'stipend[^\d]{0,80}?((?:rs\.?|₹|inr)?\s*\d[\d,\.]*\s*k?)',
            low,
            flags=re.IGNORECASE,
        ):
            amt = _parse_inr_amount(re.sub(r'(?:rs\.?|₹|inr)', '', m.group(1), flags=re.I))
            if amt != -1 and 1000 <= amt < min_internship_stipend_inr:
                msg = f'\n{blob[:1200]}\n\nStipend {amt} INR/month is below minimum {min_internship_stipend_inr}. Skipping!\n'
                return True, "Internship stipend below minimum", msg
        return False, None, None

    # Full-time / non-intern: look for explicit CTC or LPA ranges under min_fulltime_ctc_lpa
    ctc_blob = low
    patterns = [
        re.compile(r'(?:ctc|package|compensation|fixed\s+pay|salary)\s*[:.]?\s*(\d+(?:\.\d+)?)\s*[-–]\s*(\d+(?:\.\d+)?)\s*(?:lpa|l\.?p\.?a\.?|lakhs?\s*p\.?a\.?)'),
        re.compile(r'(?:ctc|package|compensation)\s*[:.]?\s*(\d+(?:\.\d+)?)\s*(?:lpa|l\.?p\.?a\.?|lakhs?\s*p\.?a\.?)'),
        re.compile(r'(\d+(?:\.\d+)?)\s*[-–]\s*(\d+(?:\.\d+)?)\s*lpa\b'),
        re.compile(r'(\d+(?:\.\d+)?)\s*lakhs?\s*(?:per\s*annum|p\.?a\.?|annum)\b'),
    ]
    max_lpa_seen: float | None = None
    for pat in patterns:
        for m in pat.finditer(ctc_blob):
            groups = [g for g in m.groups() if g is not None]
            try:
                nums = [float(x) for x in groups]
            except ValueError:
                continue
            hi = max(nums)
            max_lpa_seen = hi if max_lpa_seen is None else max(max_lpa_seen, hi)
    if max_lpa_seen is not None and max_lpa_seen + 1e-6 < float(min_fulltime_ctc_lpa):
        msg = f'\n{blob[:1200]}\n\nStated compensation (~{max_lpa_seen} LPA) is below {min_fulltime_ctc_lpa} LPA. Skipping!\n'
        return True, "Stated CTC below minimum LPA", msg
    return False, None, None
...
def get_job_description(
    ...
    ### Parameters:
    - `job_title`: Job title from listing (used with `config/search.py` India compensation rules).
    ### Returns:
    ...
    try:
        ...
        for word in bad_words:
            if word.lower() in jobDescriptionLow:
                ...
                break
        if not skip:
            ic_skip, ic_reason, ic_msg = india_compensation_gate(job_title, jobDescription)
            if ic_skip:
                skip = True
                skipReason = ic_reason
                skipMessage = ic_msg
...
def is_company_name_question(label: str) -> bool:
    '''True when the field is asking for employer/company name, not years at company.'''
    if 'employer' in label and ('name' in label or 'company' in label or label.strip() == 'employer'):
        return True
    if any(w in label for w in ('company', 'organization', 'organisation')):
        if any(w in label for w in ('name', 'current', 'previous', 'last', 'recent', 'present', 'most recent', 'former')):
            return True
        if label.strip() in ('company', 'employer', 'organization', 'organisation'):
            return True
    return False
...
def is_experience_duration_question(label: str) -> bool:
    ...
    if any(w in label for w in (
        'company', 'employer', 'organization', 'organisation', 'ctc', 'salary',
        'compensation', 'pay', 'notice', 'phone', 'email', 'linkedin', 'website',
        'portfolio', 'blog', 'github', 'hear', 'come across', 'referral', 'source',
    )):
        return False
    ...
    if 'experience' in label or 'years' in label:
        return True
    if ('how many' in label or 'number of' in label) and ('year' in label or 'month' in label):
        return True
    return False


def get_experience_answer(label: str) -> str:
    '''Maps experience questions to configured years/months (5 months professional experience).'''
    if 'month' in label:
...
def get_text_answer_fallback(label: str) -> str:
    ...
    if is_experience_duration_question(label):
        return get_experience_answer(label)
    if any(w in label for w in ('website', 'portfolio', 'blog', 'github')):
        return website
    if ('hear' in label or 'come across' in label) and 'this' in label:
        return referral_source
    return ""


def sanitize_text_answer(label: str, answer: str) -> str:
    '''Fix common wrong AI or fallback answers before typing into the form.'''
    if not answer:
        return answer
    answer = str(answer).strip()
    label_l = label.lower()
    bot_repo_markers = ('godsscion', 'auto_job_applier_linkedin')
    if any(m in answer.lower() for m in bot_repo_markers):
        if any(w in label_l for w in ('website', 'portfolio', 'blog', 'github', 'url', 'link')):
            return website
        return referral_source
    if is_company_name_question(label_l):
        if answer.isdigit() or answer in ('0', '0.0'):
            return recent_employer
        return answer
    if is_experience_duration_question(label_l) and ' in ' not in label_l:
    ...
    for Question in all_questions:
        ...
        if select:
            ...
            if overwrite_previous_answers or selected_option == "Select an option":
                ...
                # Add location handling
                    ...
                    if 'permanent' in label:
                        if 'country' in label:
                            answer = country
                        elif 'state' in label:
                            answer = permanent_state
                        elif 'city' in label:
                            answer = permanent_city
                        else:
                            answer = permanent_city if permanent_city else work_location
        ...
        if text: 
            ...
            if not prev_answer or overwrite_previous_answers:
                if is_company_name_question(label): answer = recent_employer
                    ...
                    if 'street' in label or 'line 1' in label or 'address line 1' in label:
                        answer = permanent_street
                    elif 'city' in label or 'town' in label:
                        answer = permanent_city
                    elif 'state' in label or 'province' in label:
                        answer = permanent_state
                    elif 'zip' in label or 'postal' in label:
                        answer = permanent_zipcode
                    elif 'country' in label:
                        answer = country
                    elif 'address' in label:
                        answer = permanent_address_full
                    else:
                        answer = permanent_city
                    ...
                    elif 'employer' in label or 'company' in label: answer = recent_employer
                ...
                elif any(w in label for w in ('website', 'blog', 'portfolio', 'github')): answer = website
                elif 'scale of 1-10' in label: answer = confidence_level
                elif 'headline' in label: answer = linkedin_headline
                elif ('hear' in label or 'come across' in label) and 'this' in label and ('job' in label or 'position' in label): answer = referral_source
                    ...
                    if use_AI and aiClient:
                        try:
                            if ai_provider.lower() == "openai":
                                ...
                                answer = get_text_answer_fallback(label)
                        ...
                        answer = get_text_answer_fallback(label)
                ##<
                answer = sanitize_text_answer(label, answer)
                text.clear()
        ...
        if text_area:
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                elif 'permanent' in label and 'address' in label: answer = permanent_address_full
                if answer == "":
                    if use_AI and aiClient:
                        try:
                            if ai_provider.lower() == "openai":
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    description, experience_required, skip, reason, message = get_job_description(title)
                    if skip:
                        print_lg(message)
