...
def get_installed_chrome_major_version() -> int | None:
    '''
    Detects the installed Google Chrome browser's MAJOR version number
    on Windows, macOS, and Linux. Returns an `int` (e.g. 148) on success,
    or `None` if Chrome / version info couldn't be located.

    Used so we can pin `undetected_chromedriver` via `version_main=...`
    and avoid the common error:
        "This version of ChromeDriver only supports Chrome version N
         Current browser version is N-1.x.x.x"
    '''
    import re
    import subprocess
    ...
    try:
        if sys.platform.startswith("win"):
            # 1) Try the Windows registry (works for both user & system installs)
            try:
            ...
            # 2) Fallback: read the file version of chrome.exe via PowerShell
            candidate_paths = [
            ...
            for cmd in ("google-chrome", "google-chrome-stable", "chromium", "chromium-browser"):
                try:
                    out = subprocess.check_output(
                        [cmd, "--version"], stderr=subprocess.DEVNULL, timeout=10
                    ).decode(errors="ignore").strip()
                    major = _parse_major(out)
                    if major:
                        return major
            ...
            if os.path.exists(mac_chrome):
                try:
                    out = subprocess.check_output(
                        [mac_chrome, "--version"], stderr=subprocess.DEVNULL, timeout=10
                    ).decode(errors="ignore").strip()
                    major = _parse_major(out)
                    if major:
                        return major
